...
function makeVisits(dwells, persona) {
  // Most slides visited once. Champions and engaged occasionally revisit a
  // slide (going back to look at something twice).
  const out = {};
  for (const k of Object.keys(dwells)) out[k] = 1;
  if (persona === "champion") {
    // Pick 2-3 slides they revisited.
    const slides = Object.keys(dwells);
    const revisitCount = Math.min(3, Math.max(2, Math.floor(slides.length * 0.1)));
    for (let i = 0; i < revisitCount; i++) {
      const k = pick(slides);
      out[k] = (out[k] || 1) + rand(1, 2);
    }
  } else if (persona === "engaged" && Math.random() > 0.6) {
    const slides = Object.keys(dwells);
    const k = pick(slides);
    out[k] = (out[k] || 1) + 1;
  }
...
function buildSessions(linkRows) {
  ...
  linkRows.forEach((link, idx) => {
    ...
    spec.fps.forEach((fp, fpIdx) => {
      ...
      for (let i = 0; i < sessionsForFp; i++) {
        ...
        const visits = makeVisits(dwells, spec.persona);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const visits = makeVisits(dwells, persona);
...
// Anchor STATE on globalThis so dev-mode HMR / Turbopack reloads don't
// reseed the data (which would invalidate the link IDs sessions reference).
// Bump the version suffix when the shape changes to force a fresh seed.
const STATE_KEY = "__pfa_demo_state_v2__";
if (!globalThis[STATE_KEY]) {
  globalThis[STATE_KEY] = init();
}
...
export const demo = {
  ...
  getSession(id) {
    ...
    let linkStats = null;
    if (link) {
      const real = STATE.sessions.filter((x) => x.link_id === link.id && !x.is_bot);
      linkStats = {
        visitors: new Set(real.map((x) => x.fp_hash).filter(Boolean)).size,
        sessions: real.length,
        totalSeconds: real.reduce((a, x) => a + (Number(x.total_seconds) || 0), 0),
      };
    }
    return { session: s, link: link || null, linkStats, related, events: [] };
  },
  activity({ days = 30 } = {}) {
    const since = Date.now() - days * 86_400_000;
