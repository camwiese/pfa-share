...
function linkOpenedEmail({ link, session, adminPreview = false }) {
  const where = session?.geo?.city ? ` from ${session.geo.city}${session.geo.country ? ", " + session.geo.country : ""}` : "";
  const subject = adminPreview
    ? `[Preview] ${link.name} link opened`
    : `${link.name} just opened the deck`;
  const previewNote = adminPreview
    ? `<p style="margin-top:14px;color:#7b8e80;font-size:12px;">This was an admin preview — the open is not recorded in analytics.</p>`
    : "";
  return {
    subject,
    html: shellHtml(`
...
export async function notifyLinkOpened({ link, session, firstSession = false, adminPreview = false }) {
  try {
    const service = createServiceClient();
    const settings = await getSettings(service);
    if (!settings.email_on_link_open) return { skipped: true };
    if (!settings.email_on_link_open_every && !firstSession) return { skipped: true };
    // Admin previews use a separate throttle key so a real visitor open
    // can still fire even if we just sent ourselves a preview moments ago
    // (and vice versa).
    const triggerKey = adminPreview ? `link-open-preview:${link.id}` : `link-open:${link.id}`;
    if (!(await shouldSend(service, { triggerKey, gapSeconds: settings.notification_min_gap_seconds || 60 }))) {
      return { throttled: true };
    }
    const tmpl = linkOpenedEmail({ link, session, adminPreview });
    const result = await sendViaResend({ to: await recipients(service), ...tmpl });
    await recordSent(service, {
      triggerKey,
      kind: "link_open",
      meta: { link_id: link.id, session_id: session?.id, adminPreview },
    });
    return result;
  } catch (err) {
