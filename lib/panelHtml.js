...
function ppie({ idx, base, text, alt, dims, textBelow = false }) {
  const figure = `<figure class="ppie__figure" data-animate="${textBelow ? 1 : 2}">${img({ base, alt, dims, sizes: "(max-width: 860px) 92vw, 1000px", className: "ppie__image" })}</figure>`;
  const para = `<p class="ppie__text" data-animate="${textBelow ? 2 : 1}">${text}</p>`;
  const inner = textBelow ? `${figure}${para}` : `${para}${figure}`;
...
const PANELS_HTML = [
  ...
  `<section class="panel panel--cream panel--welcome" data-panel="5">
    <div class="welcome__content">
      ...
      <figure class="welcome__figure" data-animate="1">${img({ base: "Park-Lagoon-2", alt: "The Palace of Fine Arts and lagoon, reimagined", dims: { w: 1672, h: 941 }, sizes: "(max-width: 860px) 90vw, 760px", className: "welcome__image" })}</figure>
    </div>
  </section>`,
  // 6
