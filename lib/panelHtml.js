...
const PANELS_HTML = [
  ...
  `<section class="panel panel--cream panel--welcome" data-panel="5">
    <div class="welcome__content">
      ...
      <figure class="welcome__figure" data-animate="1">${img({ base: "map-grounds-v3", alt: "Map of the Palace of Fine Arts grounds", dims: { w: 1672, h: 941 }, sizes: "(max-width: 860px) 90vw, 760px", className: "welcome__image" })}</figure>
    </div>
  </section>`,
  // 6
...
export function getPanelsHtml(start = 0, count = PANELS_HTML.length) {
  ...
  const panels = PANELS_HTML.slice(start, end).join("\n");
